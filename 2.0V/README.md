...
| 기능 | 설명 |
|------|------|
| **전용 OCR 엔진** | `oneocr.dll` 기반의 고성능 OCR 엔진 |
| **YOLOv8 객체 탐지** | 이미지에서 번호판 영역을 자동 검출 |
| **GUI 모드** | Tkinter 기반 데스크톱 앱, 이미지 직접 선택 |
...
| **DLL 자동 추출** | Windows 앱에서 OCR 엔진 자동 추출 |
...
[Releases](https://github.com/hohofught/ocrtest/releases) 페이지에서 `주차단속시스템.exe` 다운로드 후 더블클릭
...
웹 서버를 함께 시작하여 외부 기기에서 접속하는 것도 가능합니다.
